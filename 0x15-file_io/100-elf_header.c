 ...
 * @e_type: The ELF type.
 * @e_ident: A poiter to an array containing the ELF class.
 */
void print_type(unsigned int e_type, unsigned char *e_ident)
{
 ...
 * Description: If the file cann't be closed - exit code 98.
 */
void close_elf(int elf)
{
if (close(elf) == -1)
...
/**
 * main - Display the information contained in the ELF header @ the start
 * @argc: The number of argument supplied to the program.
 * @argv: An array of pointer to the arguments.
 * Return: 0 on success.
 * Description: If the file isn't ELF file or the function fails - exit code 98
 */
...
header = maloc(sizeof(Elf64_Ehdr));
if (header == NULL)
{
close_elf(o);
dprintf(STDERR_FILENO, "Error: Can't read file %s\n", argv[1]);
...
r = read(o, header,, sizeof(Elf64_Ehrd));
if (r == -1)
{
free(header);
close_elf(o);
